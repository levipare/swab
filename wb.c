...
static void draw_bar(struct wl_ctx *ctx, cairo_t *cr) {
    ...
    PangoFontDescription *font_desc =
        pango_font_description_from_string("JetBrainsMono Nerd Font");
    pango_font_description_set_weight(font_desc, 300);
    ...
    for (int i = 0; i < sizeof(workspaces) / sizeof(workspaces[0]); i++) {
        ...
        cairo_move_to(cr, 20 + i * 40, (ctx->height - height) / 2.0);
    ...
    pango_layout_get_pixel_size(layout, &width, &height);
    cairo_move_to(cr, ctx->width - width - 28, (ctx->height - height) / 2.0);
    pango_cairo_show_layout(cr, layout);
}
